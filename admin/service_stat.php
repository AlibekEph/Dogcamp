...
<div ng-app="Stat_Service" ng-controller="statCtrl">
  <input type="hidden" id="service-type" value="<?=$_GET['id']?>">
    ...
    <div class="modal-dialog">
      <div class="modal-content bg-info">
        <div class="modal-header">
          <h4 class="modal-title">{{modal_val[0][1]}} {{month[modal_val[0][0]]}}</h4>
        ...
        <div class="modal-body">
          ...
          <a style="color: #fff" href="<?=PROTOCOL?>://<?=DOMAIN?>/admin/order.php?id={{order}}"><h5>ID {{order}}</h5></a>
            </div>
        </div>
      </div>
      <!-- /.modal-content -->
            ...
            <div class="card">
              ...
              <div class="card-body">
                <table class="table" border=1 >
                  <tr>
                    <th>Месяцы</th>
                    <th ng-repeat="(_, month_num) in result">{{month[month_num[0]]}}</th>
                  </tr>

                  <tr ng-repeat="day_num in days" >
                  <td>{{day_num + 1}}</td>
                  <td class='text-center'  ng-repeat="(month, month_val) in result" ng-class="{'bg-yellow': have_houses(month_val[1][day_num])}" ng-click="open_modal(month_val[1][day_num], [month, day_num])" >
                        {{have_houses(month_val[1][day_num])? 'бронь' : ''}}
                  </td>
                  </tr>
...
<script type="text/javascript" src="dist/js/stat_service.js"></script>
